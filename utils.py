"""dSource discovery and name resolution.

Pure service layer — raises domain exceptions; never calls pytest directly.
The test layer translates these into pytest.fail / pytest.skip as appropriate.
"""
...
from e2e.constants import DSOURCE_NAME_STATE_KEY
...
def discover_dsource_name(
    orcv2, dlx: DelphixClient, pdb_subscription_id: str,
) -> str:
    ...
    Raises:
        DsourceNotFoundError: when no ACTIVE dSource matches.
    """
    pdb = orcv2.get_subscription(pdb_subscription_id)
...
def resolve_dsource_name(
    state: dict[str, Any], orcv2, dlx: DelphixClient,
) -> str:
    ...
    """
    if cached := state.get(DSOURCE_NAME_STATE_KEY):
        return cached
    if override := os.getenv("DSOURCE_NAME"):
        return override

    name = discover_dsource_name(
        orcv2, dlx, os.environ["PDB_SUBSCRIPTION_ID"],
    )
    state[DSOURCE_NAME_STATE_KEY] = name
    return name
